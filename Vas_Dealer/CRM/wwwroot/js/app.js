(function ($) {
    ...
    $('#showGroupChecked').on('click', function () {
        var checkShow = document.querySelectorAll('#haveShowing .collapse.show');
        if (checkShow.length > 0) {
            $('#haveShowing').removeClass('border-top');
            return;
        }
        $('#haveShowing').addClass('border-top');
    });
    ////////////////////////////// change status /////////////////////////////
    $('ul.dropdown-agent-status li a').click(function () {
        var $this = $(this);
        var code = $this.data('code');
        var name = $this.data('name');
        var isPause = $this.data('status') === 'True';
        $.ajax({
            type: "GET",
            url: "/pause/" + isPause + "/" + code,
            headers: headers,
            success: function (msg) {

                $('div.button-agent-status button').removeAttr('disabled');
                var item = $('a[data-code="' + code + '"]');
                if (isPause) {
                    $('div.button-agent-status button').removeClass().addClass('btn btn-danger dropdown-toggle');
                    $('div.button-agent-status button').html('<i class="icon-control-pause"></i> ' + item.data('name'))
                } else {
                ...
                //var spinHtml = '<div class="spinner-border spinner-border-sm text-light" role="status"><span class="sr-only">Loading...</span></div> ';
                //var $bg = isPause ? 'btn btn-danger dropdown-toggle' : 'btn btn-info dropdown-toggle';
                //$('div.button-agent-status button').removeClass().addClass($bg).html(spinHtml + name).attr('disabled');
            },
            error: function (msg) {
                console.log(msg);
                $.confirm({
    ...
    ///////////////////////////// Lấy danh sách cuộc gọi nhỡ trên icon //////////////////////////
    GetMissCallOnDay(headers);

    ///////////////////////////// Lấy trạng thái hiện tại của softphone //////////////////////////
    GetPeerStatus(headers);
    ...
    $('#agent-status-option').flagStrap({
        countries: {
            "1": "<i class='icon-check mr-2'></i> Sẵn sàng <span>00:00:20</span>",
            "2": "<i class='icon-user mr-2'></i> Nghỉ trưa",
            "3": "<i class='icon-user mr-2'></i> Nghỉ tối",
            "4": "<i class='icon-user mr-2'></i> Đi họp",
            "5": "<i class='icon-user mr-2'></i> Rời khỏi thành phố",
            "6": "<i class='icon-user mr-2'></i> Trực Chat",
            "7": "<i class='icon-user mr-2'></i> Xử lý email"
        },
        buttonSize: "btn-sm",
        ...
        placeholder: {
            value: "0",
            text: "<i class='icon-user mr-2'></i> Chưa sẵn sàng"
        }
    });
 ...
 * Get miss call
 * */
function GetMissCallOnDay(headers) {
    var rowTemplate = '<li><a class="dropdown-item px-2 py-2 border border-top-0 border-left-0 border-right-0" href="javascript:void(0)">' +
        '<div class="media"><img src="/images/misscall.png" alt="{PhoneNumber}" class="d-flex mr-3 img-fluid rounded-circle w-50">' +
        '<div class="media-body"><h6 class="mb-0 text-success">{Extension}: {PhoneNumber}</h6>{MissCallDate}</div></div></a></li>',
        $panel = $('ul.miss-call-panel'),
        $missCallCounter = $('span.misscall-counter');
    $.ajax({
        type: "GET",
        url: "/Agent/MissCallOnDay",
        headers: headers,
        success: function (msg) {
            console.log(msg);
            $.each(msg, function () {
                $panel.prepend(rowTemplate.supplant(this));
            });
            $missCallCounter.html(msg.length);
        }
    });
}
 ...
 * 
 * */
function GetPeerStatus(headers) {
    var exten = $('.sp-agent-exten').html();
    $.ajax({
        type: "GET",
        url: "/Agent/PeerStatus/" + exten,
        headers: headers,
        success: function (msg) {
