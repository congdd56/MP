...
function GetCode(item) {
    ...
    $.ajax({
        type: 'GET',
        url: '/Account/GetCode/' + item
    }).done(function (msg) {
        debugger;
        $('button.code').attr('uuid', msg.value.UserName);
        $('.lockscreen-name').html('<code>' + item) + '</code>';
        $('.number-one').remove();
        $('.code-panel').show('slow');
        showHideHandlingUser(false);
    }).fail(function (msg) {
        toastr.warning(msg.value.Msg, 'Cảnh báo');
        showHideHandlingUser(false);
    });
}
...
function Recovery(uuid, code, pw) {
    ...
    $.ajax({
        type: 'GET',
        url: '/account/Recoverypw/' + uuid + '/' + code + '/' + pw
    }).done(function (msg) {
        $.confirm({
            type: 'green',
        ...
        toastr.warning(msg.responseJSON.value, 'Thông báo');
